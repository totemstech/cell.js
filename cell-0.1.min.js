...
return that;else{var next=comps.shift();return my.children[next].find(comps.join('/'));}};CELL.method(that,'build',build,_super);CELL.method(that,'refresh',refresh,_super);CELL.method(that,'find',find,_super);CELL.getter(that,'children',my,'children');CELL.getter(that,'element',my,'element');CELL.getter(that,'path',my,'path');CELL.getter(that,'json',my,'json');return that;};