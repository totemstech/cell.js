...
var CELL={};CELL.cell=function(spec,my){var _super={};var my=my||{};my.element=null;my.path=spec.path||'/';my.children={};my.json={};var build;var refresh;var find;var bind;var that=emitter({});build=function(){throw new Error('`build` must be implemented : '+my.path);my.element=$('<div></div>');my.children['foo']=foocell({path:my.path+'/foo'});my.element.append(my.children['foo'].build());return my.element;};bind=function(){my.hdlr=my.hdl||function(path){that.emit('update',path);};for(var c in my.children){if(my.children.hasOwnProperty(c)){my.children[c].off('update',my.hdlr);my.children[c].on('update',my.hdlr);}}};refresh=function(json){my.json=json;for(var c in my.children){if(my.children.hasOwnProperty(c))
