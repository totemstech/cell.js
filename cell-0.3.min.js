...
my.children[c].refresh(my.json[c]);}};find=function(path){CELL.debug(my.path+' find '+path);path=path.replace(/^\/+/,'');var comps=path.split('/');var next=comps.shift();if(next.length>0)
...
bind();};find=function(path){CELL.debug(my.path+' find '+path);path=path.replace(/^\/+/,'');var comps=path.split('/');var next=comps.shift();if(next.length>0)
return my.children[next].find(comps.join('/'));else
